/**
 * Variant matrix engine — pure, no DB, no React.
 *
 * WHY THIS EXISTS
 *   A shop receives 100 pairs of shoes that must be split across 3 colours and
 *   5 sizes. The natural way to enter that is a grid, but the storage shape that
 *   billing already understands is a flat array of {size, color, stock}. This
 *   module is the translation layer, plus the bulk operations that stop the user
 *   from typing 15 numbers by hand.
 *
 * THE CELL KEY CONTRACT
 *   A cell is keyed `${color}||${size}`, and an ABSENT AXIS IS THE EMPTY STRING.
 *   That is not arbitrary: sale.service.js finds a variant with
 *       v.size === size && v.color === color
 *   where the cart defaults both to ''. A size-only product must therefore store
 *   color:'' — anything else (null, undefined, omitted) silently fails to match
 *   at the counter and the sale is rejected as "variant not found".
 *
 * PRICING FIELDS ARE NULL, NEVER ZERO
 *   null means "inherit the product-level price". A 0 would sell the variant for
 *   free. toVariantStock() therefore omits null keys entirely, so a uniformly
 *   priced product persists exactly the same document shape it always has.
 *
 * IMMUTABILITY
 *   Every operation returns a new matrix. React state updates depend on it, and
 *   an in-place edit would skip re-renders and silently show stale totals.
 */
 ...
 */
function buildMatrix({ colors = [], sizes = [], existing = {} } = {}) {
...
function matrixTotals({ rows = [], cols = [], cells = {} } = {}) {
  const rowTotals = {};
  const colTotals = {};
  let grandTotal = 0;
...
function setCell(matrix, color, size, patch) {
...
function fillRow(matrix, color, qty) {
  const value = toInt(qty);
  const cells = cloneCells(matrix.cells);
 ...
 */
function distributeEvenly(matrix, total) {
...
function copyRow(matrix, fromColor, toColors = []) {
  const cells = cloneCells(matrix.cells);
  for (const size of matrix.cols) {
 ...
 */
function toVariantStock({ rows = [], cols = [], cells = {} } = {}) {
  const out = [];
  for (const color of rows) {
    for (const size of cols) {
 ...
 */
function fromVariantStock(variantStock) {
 ...
 */
function findDuplicateCombos(variantStock) {
  const seen = new Set();
  const dupes = new Set();
